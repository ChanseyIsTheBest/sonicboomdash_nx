/* sd_tate.c -- portrait presentation, from bloonspop_nx's bp_tate.c (itself
 * clayjam_nx's), via sonicdash_nx. config.txt's rotation, read at run time:
 *   0  landscape 16:9 (default): every entry point here is inert -- the engine
 *      renders straight to the window, as in Sonic Dash 1. SD2 declares
 *      portrait only (manifest, PlayerSettings) but is played landscape here;
 *   1  portrait, turned 90 CW  onto the whole window -- console held vertically;
 *   2  portrait, turned 90 CCW onto the whole window;
 *   3  portrait, upright and centred: drawn 1:1 into a column in the middle of
 *      the window, black either side (new for SD2 -- the fallback if the
 *      landscape layout misbehaves).
 * Display transform and pointer inverse stay side by side, as below, for all
 * modes. The original header follows. */
...
static int    s_rot = 1;              /* 1 = 90 CW, 2 = 90 CCW, 3 = upright column */
...
int sd_tate_init(int render_w, int render_h, int window_w, int window_h, int rot)
{
    ...
    if (!sd_cfg_rotation) return 0;    /* rotation 0: landscape, no FBO, no extra pass */
    ...
    debugPrintf("[tate] portrait ready: engine renders %dx%d -> window %dx%d, "
                ...
                s_rot == 3 ? "upright, centred" : s_rot == 2 ? "90 CCW" : "90 CW",
...
static const GLfloat kUvId[8]  = {  0.f, 0.f,   1.f, 0.f,   0.f, 1.f,   1.f, 1.f };   /* upright */

/* UPRIGHT (rot=3): the portrait image keeps its orientation and fills a column
 * of the window's full height, centred: width cw = rw * wh / rh (== rw when the
 * render height equals the window height, which config.txt guarantees -- a 1:1
 * copy), left edge x0 = (ww - cw) / 2. The pointer inverse subtracts x0 and
 * scales back; a touch in the black side bars lands OUTSIDE the game's screen
 * (x < 0 or x >= rw) rather than being clamped onto its edge buttons. */
static void column_geometry(float rw, float rh, float ww, float wh, float *x0, float *cw) {
    /* Rounded ONCE, here, so the viewport and the pointer inverse use the very
     * same integers (720p: 405 wide at x0 = 438 -- not 437.5 in one and 438 in
     * the other). */
    *cw = (float)(int)(rw * wh / rh + 0.5f);
    *x0 = (float)(int)((ww - *cw) * 0.5f + 0.5f);
}
                         ...
                         float *out_x, float *out_y)
{
    ...
    if (sd_cfg_rotation == 3) {               /* upright column (also before init) */
        const float wh = s_ready ? (float)s_wh : rh;
        const float ww = s_ready ? (float)s_ww : rh * 16.f / 9.f;
        float x0, cw;
        column_geometry(rw, rh, ww, wh, &x0, &cw);
        *out_x = (px * (ww / panel_w) - x0) * (rw / cw);
        *out_y =  py * (rh / panel_h);
        return;
    }
    if (!s_ready || !sd_cfg_rotation) {       /* landscape: plain stretch, no rotation */
        *out_x = px * (rw / panel_w);
        *out_y = py * (rh / panel_h);
        return;
    }
...
void sd_tate_present(void)
{
    ...
    if (s_rot == 3) {                          /* upright: black bars, centred column */
        float x0, cw;
        column_geometry((float)s_rw, (float)s_rh, (float)s_ww, (float)s_wh, &x0, &cw);
        glViewport(0, 0, s_ww, s_wh);
        glClearColor(0.f, 0.f, 0.f, 1.f);
        glClear(GL_COLOR_BUFFER_BIT);
        glViewport((GLint)x0, 0, (GLsizei)cw, s_wh);
    } else {
        glViewport(0, 0, s_ww, s_wh);
    }
    ...
    const GLfloat *uv = (s_rot == 3) ? kUvId : (s_rot == 2) ? kUvCcw : kUvCw;
...
/* Rotation 0 is landscape straight to the window; 1-3 use the portrait FBO. */
...
void sd_tate_swap_hook(void) {
  ...
  if (!sd_tate_active()) {
    ...
    if (!s_logged) {
      debugPrintf("[tate] real window %ux%u, engine renders %dx%d, rotation %d (%s)\n",
                  ww, wh, screen_width, screen_height, sd_cfg_rotation,
                  sd_cfg_rotation == 3 ? "portrait, upright, centred column" : "portrait, turned 90 degrees");
      s_logged = 1;
    }
