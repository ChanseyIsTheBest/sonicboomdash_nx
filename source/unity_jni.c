...
#include "sd_pack.h"      /* AssetManager opens are served from the asset pack */
#include "libc_shim.h"    /* close_fake / fopen_fake / fclose_fake */
#include "unity_jni.h"
#include "util.h"
#include "config.h"   /* SD_PACKAGE etc. */
...
typedef struct {
  ...
  void *src;                /* UJ_SCANNER: the UJ_INPUTSTREAM it reads      */
} UHandle;
...
void unity_handle_free(void *p){
  ...
  if (h->fp) fclose_fake(h->fp);            /* frees a packed file's memory stream */
  if (h->fd>=0) close_fake(h->fd);          /* returns a pack handle to the pack */
  free(h);
}
...
/* config.txt resolution -> Unity's saved screen size (bloonspop_nx's guard).
 * Unity can save its last screen size in PlayerPrefs; builds before config.txt
 * ran at 1920x1080, so a saved entry could pull that size back against the new
 * window. Only keys the game already wrote are touched -- in either key form,
 * plain or URI-encoded, since Unity encodes PlayerPrefs keys on Android. */
static void prefs_sync_screen_size(void){
  extern int screen_width, screen_height;
  static const char *const K[4] = {
    "Screenmanager Resolution Width",  "Screenmanager%20Resolution%20Width",
    "Screenmanager Resolution Height", "Screenmanager%20Resolution%20Height" };
  char v[2][16];
  snprintf(v[0], sizeof v[0], "%d", screen_width);
  snprintf(v[1], sizeof v[1], "%d", screen_height);
  int changed = 0;
  for (int i = 0; i < 4; i++) {
    KV *e = kv_get(K[i]);
    if (e && strcmp(e->val, v[i / 2])) { kv_set('I', K[i], v[i / 2]); changed++; }
  }
  if (changed) {
    prefs_flush();
...
/* The Java class a boxed value reports from getClass(): Unity's Unbox switches on
 * getClass().getName() == "java.lang.Integer" etc. before calling intValue(). */
const char *unity_boxed_class(void *p){
  UHandle*h=p; if(!is_uh(h,UJ_BOXED)) return NULL;
  switch(h->btype){ case 'I': return "java/lang/Integer"; case 'L': return "java/lang/Long";
                    case 'B': return "java/lang/Boolean"; case 'F': return "java/lang/Float"; }
...
#if SD_JNI_FIX_SCANNER
/* ---- java.util.Scanner over an asset InputStream ---------------------------
 * FOUND IN THE LOG: Unity reads bin/Data/boot.config as
 *     new Scanner(assetManager.open("bin/Data/boot.config")).useDelimiter("\\A").next()
 * -- the Java idiom for "the whole stream as one string". There was no Scanner,
 * so useDelimiter returned a pooled object and next() the generic "", and the
 * engine started every boot with an EMPTY boot.config (gfx-threading-mode,
 * androidStartInFullscreen, androidRenderOutsideSafeArea, build-guid -- the
 * last is Application.buildGUID). A Scanner is a handle over the stream it
 * was built on; next() returns the rest of the stream (the \A delimiter's
 * meaning -- the only one Unity uses), nextLine() one line. */
void *unity_new_scanner(void *stream){
  UHandle*h=uh_new(UJ_SCANNER);
  h->src = is_uh(stream,UJ_INPUTSTREAM) ? stream : NULL;
  if (!h->src) debugPrintf("[jni] new Scanner(%p): not one of our InputStreams -- it will read empty\n", stream);
  return h;
}
static FILE *scanner_fp(void *recv){
  UHandle*h=recv; if(!is_uh(h,UJ_SCANNER) || !h->src) return NULL;
  return ((UHandle*)h->src)->fp;
}
static void *scanner_read(void *recv, int line){
  FILE*fp=scanner_fp(recv); if(!fp) return jni_make_string("");
  size_t cap=4096, n=0; char*b=malloc(cap); if(!b) return jni_make_string("");
  int c;
  while((c=fgetc(fp))!=EOF){
    if(line && c=='\n') break;
    if(n+2>cap){ char*nb=realloc(b,cap*=2); if(!nb) break; b=nb; }
    b[n++]=(char)c;
  }
  if(line && n && b[n-1]=='\r') n--;
  b[n]=0;
  void*r=jni_make_string(b);
  if(n<=256){ char shown[260]; size_t k=0; for(;k<n;k++) shown[k]=(b[k]=='\n'||b[k]=='\r')?'|':b[k]; shown[k]=0;
    debugPrintf("[jni] Scanner.%s -> %zu bytes: %s\n", line?"nextLine":"next", n, shown); }
  else debugPrintf("[jni] Scanner.%s -> %zu bytes\n", line?"nextLine":"next", n);
  free(b); return r;
}

#endif /* SD_JNI_FIX_SCANNER */
...
void *unity_dispatch_object(void *recv, const void *id_, va_list va){ const struct FakeID *id = id_;
  ...
  if (has(cls,"java/util/Scanner")){
    if (has(m,"useDelimiter") || has(m,"skip") || has(m,"reset")) return recv;   /* fluent: the Scanner */
    if (!strcmp(m,"nextLine")) return scanner_read(recv,1);
    if (!strcmp(m,"next"))     return scanner_read(recv,0);
    return jni_make_object("java/util/Scanner");
  }
#endif
  ...
  if (has(cls,"AssetManager")){
    if (has(m,"openFd") || has(m,"openNonAssetFd")){
      ...
      int fd = sd_pack_open_read(path);          /* the loose tree is gone once packed */
      if (fd < 0) fd = open(path,O_RDONLY);
      debugPrintf("[io] JNI AssetManager.openFd(%s) -> %s [%s]%s\n", name, fd>=0?"ok":"MISSING", path,
                  sd_is_pack_fd(fd) ? " [pack]" : "");
      if(fd<0){ return NULL; /* CHECK: engine expects exception; NULL usually ok */ }
      long flen = sd_fd_size(fd);                /* pack fds have no POSIX size */
      UHandle*h=uh_new(UJ_AFD); h->fd=fd; h->off=0; h->len=flen<0?0:flen; return h;
    }
    if (has(m,"open")){
      const char *name = jni_string_utf(va_arg(va,void*));
      char path[320]; asset_path(path,sizeof path,name);
      FILE*fp=fopen_fake(path,"rb");             /* pack-aware (memory stream when packed) */
  ...
  if (has(cls,"content/Context")){
    ...
    if (has(m,"getPackageName")) return jni_make_string(SD_PACKAGE);
...
#if SD_JNI_FIX_SCANNER
  if (has(cls,"java/util/Scanner") && (has(m,"hasNext"))){            /* hasNext / hasNextLine */
    FILE*fp=scanner_fp(recv); if(!fp) return 0;
    int c=fgetc(fp); if(c==EOF) return 0; ungetc(c,fp); return 1;
  }
#endif
...
void unity_dispatch_void(void *recv, const void *id_, va_list va){ const struct FakeID *id = id_;
  const char *cls=id->cls, *m=id->name;
#if SD_JNI_FIX_SCANNER
  if (has(cls,"java/util/Scanner") && has(m,"close")){                /* closes its source, as Java's does */
    UHandle*h=recv; if(is_uh(h,UJ_SCANNER) && h->src){ UHandle*s=h->src; if(s->fp){fclose_fake(s->fp);s->fp=NULL;} }
    return; }
#endif
...
void unity_jni_init(const char *data_root){
  ...
  prefs_load(); prefs_sync_screen_size();
