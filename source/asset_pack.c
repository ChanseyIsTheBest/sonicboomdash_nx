/* asset_pack.c -- the whole assets/ tree served from ONE file.
 *
 * From fruitninjaclassic_nx (MIT), where it is hardware-proven; carried over
 * with its on-disk format unchanged (SSNXPAK1 / SSNXIDX1, version 2).
 *
 * WHY: SD2 ships ~3,800 files in a single assets/bin/Data directory. On a
 * Switch SD card every open() of one of them is a service call plus a lookup
 * in that huge directory, and Unity opens and stats them by the thousand. At
 * first boot this module copies every file under assets/ into assets.nxpack
 * with a sorted index (assets.nxidx), verifies the written data by reading it
 * back, and mounts it. From then on a lookup is a binary search in RAM and a
 * read comes from one already-known file.
 *
 * HOW IT IS SERVED: libc_shim.c routes every path-based call (open, stat,
 * access, opendir, fopen) through nx_pack_relpath() first, and every fd-based
 * call (read, pread, lseek, fstat, close, dup, fdopen, mmap) through
 * asset_pack_fd_is(). A pack "fd" is a real newlib fd on the pack file, with
 * its own position and a 64 KB read cache kept here.
 *
 * Changes from the donor: a log line when the handle table is full (the donor
 * failed that open silently with EMFILE); 1024 handles instead of 512; and
 * normalize_asset_path keys on the FIRST "assets" segment, not the last.
 */
#include <dirent.h>
#include <errno.h>
#include <fcntl.h>
...
/* Open pack files at once. Each is also a real newlib fd on the pack file, so
 * this cap sits ON TOP of newlib's own limit; 1024 keeps the pack from being
 * the tighter of the two (the donor used 512). */
#define PACK_HANDLES 1024
...
static int normalize_asset_path(const char *input, char *output, size_t capacity,
  ...
  /* The FIRST "assets" segment is the tree root. (The donor's loop kept the
   * LAST "/assets" it found, so a packed file under any directory itself named
   * "assets" would have been looked up under the wrong key.) */
  const char *relative = NULL;
  if (!strcmp(temp, "assets")) relative = temp + 6;
  else if (!strncmp(temp, "assets/", 7)) relative = temp + 7;
  for (const char *p = temp; !relative && (p = strstr(p, "/assets")) != NULL; p++) {
...
static int load_pair(const char *pack_path, const char *index_path,
  ...
  /* NOTE (from the donor): the validation above compares the two headers'
   * data checksums to EACH OTHER. That proves both files came from one run of
   * the packer; it never hashes a payload byte here, so a pack damaged AFTER it
   * was written still mounts. The payload itself is verified once, when the
   * pack is built (verify_pack_data below). */
...
static int add_handle(int fd, uint32_t entry, uint64_t position, uint64_t directory_ino) {
  ...
  errno = EMFILE;
  { static int logged;
    if (!logged++) debugPrintf("[pack] handle table FULL (%d open) -- an asset open failed "
                               "with EMFILE; raise PACK_HANDLES in asset_pack.c\n", PACK_HANDLES); }
  return -1;
}
