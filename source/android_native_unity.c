...
#include <switch.h>
#include "sd_tilt.h"
#include "sd_config.h"   /* sd_cfg_rotation */
...
void android_native_update_mode(void){
  ...
  /* The WINDOW is always the landscape 16:9 swapchain (see main.c / sd_tate.c):
   *   rotation 0:   landscape render == the window (SD1's path, no FBO);
   *   rotation 1/2: the portrait render turned onto the window -> swapped axes;
   *   rotation 3:   the portrait column sits upright in a 16:9 window of the
   *                 same height. */
  if (sd_cfg_rotation == 1 || sd_cfg_rotation == 2) { g_win_w = g_h;            g_win_h = g_w; }
  else if (sd_cfg_rotation == 3)                    { g_win_w = g_h * 16u / 9u; g_win_h = g_h; }
...
int32_t  ANativeWindow_setBuffersGeometry(ANativeWindow *w, int32_t width, int32_t height, int32_t format){
   ...
   * showed the splash zoomed in: Sonic Dash 1 asked for a 75% render scale
...
void android_native_feed_hid(void){
  ...
  sd_tilt_update();                 /* Enerbeam tilt, from the pad/sensors nxp_poll just read */
