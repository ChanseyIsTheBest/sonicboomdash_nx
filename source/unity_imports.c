...
#include "sd_pack.h"      /* pack-aware dup / fdopen */
...
int z_dup(int fd){
  if (sd_is_pack_fd(fd)) {                      /* a packed file: duplicate the pack handle */
    int n = asset_pack_dup_fd(fd);
    debugPrintf("[io] dup(%d) -> %d [pack]\n", fd, n);
    return n;
  }
