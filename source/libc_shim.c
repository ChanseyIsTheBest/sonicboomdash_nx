...
#include "diag.h"
#include "asset_pack.h"   /* the assets/ tree served from one file (SD_ASSET_PACK) */
#include "sd_pack.h"
...
/* Both used to call newlib read()/lseek() directly, bypassing read_fake -- and
 * with it the short-read loop, and now the asset pack: a fortified read of a
 * packed file would have read the PACK at the handle's raw position. */
long  __read_chk_fake(int fd, void *buf, size_t count, size_t buflen) { (void)buflen; return read_fake(fd, buf, count); }
long  __pread_chk_fake(int fd, void *buf, size_t count, long off, size_t buflen) {
  (void)buflen;
  return sd_pread(fd, buf, count, off);
}
...
long z_lseek(int fd, long off, int whence) {
  if (asset_pack_active() && asset_pack_fd_is(fd)) return asset_pack_lseek_fd(fd, off, whence);
  struct RaCache *c = ra_find(fd);
...
static const char *casetest_redirect(const char *path) {
  ...
  if (strncasecmp(b, "CASESENSITIVETEST", 17) == 0) {
    static char scratch[300];
    snprintf(scratch, sizeof scratch, "%s/.casetest", GAME_HOME);
    return scratch;
  }
  return path;
}
...
/* ==========================================================================
 * Asset pack routing (asset_pack.c, from fruitninjaclassic_nx). Once the pack
 * is mounted the loose assets/ tree is deleted (SD_PACK_DELETE_LOOSE), so EVERY
 * entry point the game can reach a data file through must consult it first:
 *   by path: open/openat/__open_2, stat/lstat, access, opendir, fopen
 *   by fd:   read/__read_chk, pread/__pread_chk, lseek/lseek64, fstat, close,
 *            dup, fdopen, mmap, readdir/closedir
 * Anything not in the pack falls through to the SD card (runtime-written
 * files, the directory skeleton), exactly as before.
 * ========================================================================== */

/* Reduce any spelling of a game path to the pack key "assets/...". Three forms
 * reach us (the donor found each the hard way):
 *   sdmc:/switch/<folder>/assets/...   what the loader and sd_remap_path build
 *   /switch/<folder>/assets/...        no device prefix -- what Unity sends
 *   assets/...                         relative to the cwd (GAME_HOME)
 * Returns NULL when the path is not a packed game path. */
const char *nx_pack_relpath(const char *path) {
  if (!path) return NULL;
  const char *p = path;
  if (!strncmp(p, "sdmc:", 5)) p += 5;
  const char *bare = sd_home_bare();              /* "/switch/<any folder>" (sd_home.c) */
  const size_t blen = strlen(bare);
  if (blen > 1 && !strncmp(p, bare, blen) && p[blen] == '/') p += blen;
  /* Separators and "./" between the folder and "assets": a joined path can
   * carry "//" or "/./", and a miss here is a missing file once the loose
   * tree is deleted. A root-relative "/assets/..." lands here too, which
   * matches sd_paths.c's rule (nothing real lives in /assets on the SD). */
  for (;;) {
    if (*p == '/') p++;
    else if (p[0] == '.' && p[1] == '/') p += 2;
    else break;
  }
  return (!strncmp(p, "assets", 6) && (p[6] == '/' || p[6] == 0)) ? p : NULL;
}
int sd_is_pack_fd(int fd) { return asset_pack_active() && asset_pack_fd_is(fd); }
int sd_pack_exists(const char *path) {
  if (!asset_pack_active()) return 0;
  const char *rel = nx_pack_relpath(path);
  uint64_t sz = 0; int isdir = 0;
  return rel && asset_pack_stat_path_info(rel, &sz, NULL, &isdir);
}
int sd_pack_open_read(const char *path) {
  if (!asset_pack_active()) return -1;
  const char *rel = nx_pack_relpath(path);
  return rel ? asset_pack_open_path(rel) : -1;
}
long sd_fd_size(int fd) {
  if (sd_is_pack_fd(fd)) { uint64_t sz = 0; return asset_pack_fstat_fd(fd, &sz, NULL, NULL) ? (long)sz : -1; }
...
long sd_pread(int fd, void *buf, size_t n, long off) {
  if (sd_is_pack_fd(fd)) {
    size_t got = 0;
    while (got < n) {
      long r = asset_pack_pread_fd(fd, (char *)buf + got, n - got, off + (long)got);
      if (r <= 0) { if (!got) return r; break; }
      got += (size_t)r;
    }
    return (long)got;
  }
...
/* A packed file opened as a FILE* (fopen, fdopen, the Java AssetManager) is a
 * read-only memory stream over the whole file (the donor's approach). The
 * buffer is tracked here so fclose_fake frees it; the donor leaked it. */
static struct { FILE *f; void *buf; } g_memf[64];
static Mutex g_memf_lock;
static FILE *memf_open(void *data, size_t size) {
  /* fmemopen cannot express an EMPTY stream (size 0 is EINVAL), and the donor's
   * 1-byte stand-in handed back one uninitialised byte. Zero it and start at
   * its end, so an empty file reads as EOF straight away. */
  if (!size) *(char *)data = 0;
  FILE *f = fmemopen(data, size ? size : 1, "r");
  if (!f) { free(data); return NULL; }
  if (!size) fseek(f, 0, SEEK_END);
  mutexLock(&g_memf_lock);
  for (unsigned i = 0; i < sizeof g_memf / sizeof *g_memf; i++)
    if (!g_memf[i].f) { g_memf[i].f = f; g_memf[i].buf = data; mutexUnlock(&g_memf_lock); return f; }
  mutexUnlock(&g_memf_lock);
  { static int logged; if (!logged++) debugPrintf("[pack] memory-stream table full: a buffer will leak at fclose\n"); }
  return f;
}
/* 1 = f was a tracked pack stream: closed and its buffer freed. */
static int memf_close(FILE *f, int *result) {
  void *buf = NULL;
  mutexLock(&g_memf_lock);
  for (unsigned i = 0; i < sizeof g_memf / sizeof *g_memf; i++)
    if (g_memf[i].f == f) { buf = g_memf[i].buf; g_memf[i].f = NULL; g_memf[i].buf = NULL; break; }
  mutexUnlock(&g_memf_lock);
  if (!buf) return 0;
  *result = fclose(f);
  free(buf);
...
static FILE *pack_fopen(const char *path) {
  if (!asset_pack_active()) return NULL;
  const char *rel = nx_pack_relpath(path);
  void *data = NULL; size_t size = 0;
  if (!rel || !asset_pack_read_all_path(rel, &data, &size)) return NULL;
  return memf_open(data, size);
}
/* fdopen() on a pack fd: the FILE takes over the fd, so the whole entry becomes
 * a memory stream at the handle's position and the pack handle is released. */
FILE *fdopen_fake(int fd, const char *mode) {
  if (sd_is_pack_fd(fd)) {
    if (strpbrk(mode, "wa+")) { errno = EINVAL; return NULL; }   /* the pack is read-only */
    uint64_t sz = 0; int isdir = 0;
    if (!asset_pack_fstat_fd(fd, &sz, NULL, &isdir) || isdir) { errno = EBADF; return NULL; }
    long pos = asset_pack_lseek_fd(fd, 0, SEEK_CUR);
    void *buf = malloc(sz ? (size_t)sz : 1);
    if (!buf) { errno = ENOMEM; return NULL; }
    if (sz && sd_pread(fd, buf, (size_t)sz, 0) != (long)sz) { free(buf); errno = EIO; return NULL; }
    FILE *f = memf_open(buf, (size_t)sz);
    if (!f) return NULL;
    if (pos > 0) fseek(f, pos, SEEK_SET);
    asset_pack_close_fd(fd);
    debugPrintf("[pack] fdopen(pack fd %d) -> memory stream, %llu bytes\n", fd, (unsigned long long)sz);
    return f;
  }
  return fdopen(fd, mode);
}
static void pack_open_note(const char *path, int fd) {
  static unsigned n;
  n++;
  if (n <= 24 || (n % 1000) == 0)
    debugPrintf("[io] open(%s) -> %d [pack] (%u pack opens so far)\n", path, fd, n);
}

/* opendir went straight to newlib, so a root-relative "/assets/..." directory
 * could never resolve. Wrapped for the same remap as open/stat -- and packed
 * directories are listed from the pack. */
DIR *opendir_fake(const char *path) {
  char b[320];
  path = sd_remap_path(path, b, sizeof b);
  if (asset_pack_active()) {
    const char *rel = nx_pack_relpath(path);
    void *pd = rel ? asset_pack_opendir_path(rel) : NULL;
    if (pd) return (DIR *)pd;
  }
  return opendir(path);
}
/* closedir was bound straight to newlib; a pack directory must go back to the pack. */
...
int open_fake(const char *path, int flags, ...) {
  char sd_rbuf_[320]; path = sd_remap_path(path, sd_rbuf_, sizeof sd_rbuf_);   /* jar:/!/assets -> SD (sd_paths.c) */
  ...
  if (!writing && asset_pack_active()) {        /* read-only game files: from the pack */
    const char *rel = nx_pack_relpath(path);
    if (rel) {
      int pfd = asset_pack_open_path(rel);
      if (pfd >= 0) { pack_open_note(path, pfd); return pfd; }
    }
  }
  ...
  if (fd >= 0) {
    ...
    if (fstat(fd, &_st) == 0) {
      ...
      if (SD_RA_CACHE && !writing && _st.st_size >= (4 << 20))   /* config.h */
...
static void pack_fill_stat(struct bionic_stat *st, uint64_t sz, uint64_t ino, int isdir) {
  memset(st, 0, sizeof *st);
  st->st_size = (int64_t)sz; st->st_ino = ino ? ino : 1;
  st->st_mode = isdir ? 0040755 : 0100644;
  st->st_nlink = 1; st->st_blksize = 4096; st->st_blocks = (int64_t)((sz + 511) / 512);
}
int stat_fake(const char *path, struct bionic_stat *st) {
  ...
  if (asset_pack_active()) {
    ...
    if (rel && asset_pack_stat_path_info(rel, &sz, &ino, &isdir)) { pack_fill_stat(st, sz, ino, isdir); return 0; }
  }
...
int fstat_fake(int fd, struct bionic_stat *st) {
  if (sd_is_pack_fd(fd)) {
    uint64_t sz = 0, ino = 0; int isdir = 0;
    if (asset_pack_fstat_fd(fd, &sz, &ino, &isdir)) { pack_fill_stat(st, sz, ino, isdir); return 0; }
  }
...
void *readdir_fake(void *dirp) {
  ...
  if (asset_pack_dir_is(dirp)) {              /* a packed directory */
    memset(&out, 0, sizeof(out));
...
/* ---- allocator-lock barrier for the GC bridge ------------------------------
 * The bridge freezes threads at arbitrary instructions. A thread frozen while
 * holding the malloc lock or this fallback-table lock would block anyone who
 * then allocates -- including the collector, for the rest of its round. The
 * bridge holds both across each pause (diag.c), so no frozen thread can own
 * one. Order fb -> malloc; nothing else nests them (g_fb_lock never wraps an
 * allocation; newlib never takes g_fb_lock). newlib's __malloc_lock ignores
 * its argument (mlock.c) and is recursive. */
struct _reent;
void __malloc_lock(struct _reent *);
void __malloc_unlock(struct _reent *);
void alloc_locks_hold(void)    { mutexLock(&g_fb_lock); __malloc_lock(NULL); }
void alloc_locks_release(void) { __malloc_unlock(NULL); mutexUnlock(&g_fb_lock); }

/* Fill [dst, dst+n) from file offset `off` of fd. A pack fd is a handle on the
 * WHOLE pack file: a raw lseek/read on it would copy pack bytes, not the
 * entry's, so it goes through the pack (entry-relative). The donor's mmap had
 * exactly that hole; a mapped packed file would have loaded the wrong bytes. */
static long mmap_fill(int fd, void *dst, size_t n, long off) {
  if (sd_is_pack_fd(fd)) {
    long r = sd_pread(fd, dst, n, off);
    return r < 0 ? 0 : r;
  }
  long got = 0, cur = lseek(fd, 0, SEEK_CUR);
  if (lseek(fd, off, SEEK_SET) >= 0)
    while ((size_t)got < n) { long r = read(fd, (char *)dst + got, n - (size_t)got); if (r <= 0) break; got += r; }
...
static void *mmap_fallback(size_t length, int flags, int fd, long offset) {
  ...
  if (flags & BIONIC_MAP_ANONYMOUS) {
    ...
    if (fd >= 0) got = mmap_fill(fd, q, length, offset);
...
void *mmap_fake(void *addr, size_t length, int prot, int flags, int fd, long offset) {
  ...
  if (g_overcommit) {
    ...
    if (!(flags & BIONIC_MAP_ANONYMOUS) && fd >= 0) (void)mmap_fill(fd, p, fill, offset);
    return p;
  }
  ...
  if (flags & BIONIC_MAP_ANONYMOUS) {
    ...
    long got = 0;
    if (fd >= 0) got = mmap_fill(fd, p, fill, offset);
...
FILE *fopen_fake(const char *path, const char *mode) {
  ...
  if (!writing) {                                  /* packed game files are read-only */
    FILE *pf = pack_fopen(path);
    if (pf) {
      static unsigned n;
      if (++n <= 16) debugPrintf("[io] fopen(%s,%s) -> %p [pack]\n", path, mode, (void *)pf);
      return pf;
    }
  }
...
int fclose_fake(FILE *f) { if (is_fake_file(f)) return 0;
  { int mr; if (memf_close(f, &mr)) return mr; }   /* a packed file's memory stream */
...
long read_fake(int fd, void *buf, size_t count) {
  ...
  if (sd_is_pack_fd(fd)) return asset_pack_read_fd(fd, buf, count);
...
int close_fake(int fd) {
  if (fd >= 0 && fd == debug_log_fd()) {         /* never let the game close our log */
    static int noted;
    if (!noted++) debugPrintf("[io] game tried to close fd %d -- that is debug.log; refused\n", fd);
    return 0;
  }
  if (sd_is_pack_fd(fd)) return asset_pack_close_fd(fd);
  ra_detach(fd);
  fd_ino_clear(fd);
...
void *getpwuid_fake(int uid) {
  ...
  snprintf(dir, sizeof dir, "%s", GAME_HOME);   /* the run-time game folder */
...
/* dlopen: every library "succeeds" with a dummy handle, and dlsym resolves
 * names against our own tables -- EXCEPT the ones below, which must genuinely
 * FAIL so that the engine takes its fallback path.
 *
 * THIS WAS NOT IMPLEMENTED BEFORE, AND THE DOCS SAID IT WAS. The inherited
 * shim returned 0x1 for every name including libvulkan.so, and
 * SD_REFUSE_VULKAN in config.h had no reader. A non-NULL handle from
 * dlopen("libvulkan.so") tells Unity Vulkan is present; whether it then
 * recovers cleanly when vkCreateInstance fails to resolve is not something to
 * rely on. Refusing the load is the path Unity's own GLES fallback is written
 * for, and it is the one clayjam_nx documents as working. */
static const char *s_dl_refused;        /* for dlerror() after a refusal */

static int dl_is_refused(const char *name) {
  if (!name) return 0;
#if SD_REFUSE_VULKAN
  /* mesa/nouveau is OpenGL-only; the Switch's GM20B has no Vulkan driver in
   * homebrew. This build ships GLES3 shaders (tools/check_graphics_api.py), so
   * the fallback has something to render with. */
  if (strstr(name, "libvulkan")) return 1;
#endif
  /* We implement OpenSL only. GetAndroidAudioOutputType is already forced to
   * OpenSL (sd_patches.c), so this is belt-and-braces: if any path still
   * probes for AAudio, "not present" is the truthful answer, and it keeps
   * FMOD from selecting an output we cannot drive. */
  if (strstr(name, "libaaudio")) return 1;
  return 0;
}

void *dlopen_fake(const char *name, int flags) {
  (void)flags;
  if (dl_is_refused(name)) {
    s_dl_refused = name;
    debugPrintf("[dl] refused %s -> NULL (engine must fall back)\n", name);   /* not "dlopen...": log_is_noisy drops those */
    return NULL;
  }
  s_dl_refused = NULL;
  debugPrintf("dlopen(%s)\n", name ? name : "(self)");
  return (void *)0x1;
}
int dlclose_fake(void *h) { (void)h; return 0; }

/* After a refused dlopen, callers commonly print dlerror(). Returning NULL
 * there is legal but prints "(null)" and hides WHY the load failed; return a
 * message that names the refusal instead. Cleared on read, as POSIX requires. */
const char *dlerror_fake(void) {
  static char msg[160];
  if (!s_dl_refused) return NULL;
  snprintf(msg, sizeof msg, "%s: refused by sonicboomdash (not available on Switch)",
           s_dl_refused);
  s_dl_refused = NULL;
  return msg;
}
/* dlsym logging, capped: an engine resolving symbols in a loop must not be
 * able to fill the SD card. */
static int dl_log_ok(void) {
  static int n;
  if (n < 3000) { n++; return 1; }
  if (n++ == 3000) debugPrintf("[dl] further dlsym logging suppressed (3000 lines)\n");
  return 0;
}
...
void *dlsym_fake(void *handle, const char *symbol) {
  ...
  if (shim) { if (dl_log_ok()) debugPrintf("dlsym(%s) -> %p [shim]\n", symbol, (void *)shim); return (void *)shim; }
  ...
  if (fb) return fb;
  /* 2c) SEGA's MariaUpload P/Invokes (sd_sdk_stubs.c). libmariaUpload.so is
   *     deliberately not staged -- it is SEGA's telemetry/auth stack with its
   *     own statically linked network code. Its 22 managed entry points are
   *     answered in-process instead, including the auth loop, which MUST
   *     return a terminal result or the boot polls it forever. */
  extern void *sd_sdk_stub_lookup(const char *symbol);
  void *sd = sd_sdk_stub_lookup(symbol);
  if (sd) return sd;
  ...
  if (!strncmp(symbol, "gl", 2) || !strncmp(symbol, "egl", 3)) {
    ...
    if (p) { if (dl_log_ok()) debugPrintf("dlsym(%s) -> %p [egl]\n", symbol, p); return p; }
  }
  if (dl_log_ok()) debugPrintf("dlsym(%s) -> NULL\n", symbol);
  return NULL;
}
...
/* The GC bridge lives in sd_gc.c now.
 *
 * WHAT WAS HERE WAS DANGEROUS AND HAD TO GO. The inherited implementation
 * carried FOUR HARDCODED OFFSETS FROM A DIFFERENT GAME (0x209fd94 and
 * friends, from the donor's libil2cpp.so) and ignored its thread argument
 * entirely -- `(void)t;`. Against Sonic Dash 1's libil2cpp those addresses pointed
 * at unrelated data, so it would have read garbage as signal numbers, matched
 * nothing, and left the first collection hanging exactly as if no bridge
 * existed. Worse, had they happened to match, it would have posted acks for
 * threads that were never stopped.
 *
 * sd_gc.c replaces it with offsets derived from THIS binary and guarded
 * against a mismatch, and it actually suspends the thread instead of only
 * acknowledging that it might have. */
#include "sd_gc.h"

int pthread_kill_gc(pthread_t t, int sig) {
  return sd_gc_pthread_kill((void *)t, sig);
}
