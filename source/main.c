/* main.c -- Sonic Dash 2: Sonic Boom, Nintendo Switch wrapper entry point.
 *
 ...
 *   asset_pack.c / sd_pack_boot.c the assets/ tree packed into one file (first boot)
 ...
 *
 * Forked from sonicdash_nx (Sonic Dash 1; itself colorsheep_nx / laytonbmr_nx /
 * vln_nx lineage, MIT). The memory/heap/overcommit scaffolding below is
 * inherited unchanged: it is an engine-generation property, not a game
 * property. The asset layout is this game's: classic player data, served from
 * the asset pack -- see check_data() and sd_pack_boot.c.
 */
#include <stdlib.h>
#include <malloc.h>
#include <string.h>
...
#include "sd_pack.h"
#include "sd_home.h"
#include "sd_uitrace.h"
#include "sd_saveedit.h"       /* the game folder, from argv[0] */       /* asset pack: sd_pack_boot, sd_pack_exists */
 ...
 * "sonicboomdash+0xOFFSET", and because the ELF is linked at address 0, OFFSET is
 * directly an address in sonicboomdash.elf (e.g. aarch64-none-elf-addr2line). */
...
static void nro_range_init(void) {
  ...
  debugPrintf("[boot] nro image 0x%lx..0x%lx -- crash addresses in it print as "
              "sonicboomdash+offset (= address in sonicboomdash.elf)\n",
...
int crash_resolve_module(uintptr_t addr, char *name_out, size_t name_cap, uintptr_t *base_out) {
  if (g_nro_base && addr >= g_nro_base && addr < g_nro_end) {
    snprintf(name_out, name_cap, "sonicboomdash");
    *base_out = g_nro_base;
    return 1;
  }
...
/* ==========================================================================
 * Data layout check, module load.
 * ========================================================================== */

/* Boot-time data check, for Sonic Dash 2's layout: classic player data under
 * assets/bin/Data/ from the base APK (no asset pack, no OBB, no Addressables),
 * plus the three libraries from the arm64 split. Once the asset pack is built
 * the loose files are deleted, so every data file is looked up in the pack
 * first (sd_pack_exists) and on the SD card second. */
static void check_data(int packed) {
  static const char *libs[] = { LIB_MAIN, LIB_UNITY, LIB_IL2CPP };
  static const char *data[] = {
    ...
    /* shipped as .split0/.split1 chunks: joined at first boot (nx_splitjoin.c) */
  ...
  for (unsigned i = 0; i < sizeof libs / sizeof *libs; i++) {
    ...
    if (stat(path, &st) < 0)
      fatal_error("Missing library:\n%s\n\nCopy libmain.so, libunity.so and libil2cpp.so from\n"
                  "your APK's lib/arm64-v8a/ (the config.arm64_v8a split) into\n%s", libs[i], DATA_ROOT);
  }
  for (unsigned i = 0; i < sizeof data / sizeof *data; i++) {
    snprintf(path, sizeof path, "%s/%s", DATA_ROOT, data[i]);
    if (!sd_pack_exists(path) && stat(path, &st) < 0)
      fatal_error("Missing data file:\n%s\n\nCopy the assets/ folder from your Sonic Dash 2 APK\n"
                  "(com.sega.sonicboomandroid) into\n%s\n\n(%s)", data[i], DATA_ROOT,
                  packed ? "an asset pack is mounted but lacks this file -- delete\n"
                           "assets.nxpack and assets.nxidx and copy assets/ again"
                         : "no asset pack yet");
  }
  debugPrintf("[boot] data ok (%s)\n", packed ? "asset pack" : "loose files");
}
...
int main(int argc, char *argv[]) {
  /* The game folder FIRST: it is wherever this .nro was started from (sd_home.c),
   * and the log file lives there, so nothing may log before this. */
  sd_home_init(argc, argv);
  debug_log_open();          /* <folder>/debug.log, or the SD root if the folder refuses it */
  socketInitializeDefault();
  debugPrintf("[boot] === sonicboomdash start (Sonic Dash 2 3.24.0, Unity 6000.0.72f1, libunity BuildID 6689ac689a52c293) ===\n");
  debugPrintf("[boot] game folder %s (from %s; argv[0]=%s)\n", sd_home(), sd_home_how(),
   ...
   * files via relative paths, so chdir into DATA_ROOT. */
  {
    ...
    struct stat st;
    int loose = stat("assets/bin/Data/globalgamemanagers", &st) == 0;   /* before packing */
    int pack  = stat("assets.nxpack", &st) == 0;                         /* after packing */
    debugPrintf("[boot] cwd '%s' -> chdir(%s)=%d; loose assets=%d, assets.nxpack=%d\n",
                cwd, DATA_ROOT, rc, loose, pack);
  }
  ...
  sd_saveedit_apply(); /* save_edit.txt: written if missing; its uncommented settings applied before the game reads its save */
   ...
   * scales the picture to the panel or the TV. The WINDOW is always landscape
   * (res*16/9 x res). What the engine renders:
   *   rotation 0 (default): landscape 16:9, res*16/9 x res (1280x720), straight
   *               to the window, exactly as Sonic Dash 1. SD2's manifest and
   *               PlayerSettings declare portrait only, but like SD1 it is
   *               played landscape here;
   *   rotation 1/2: portrait res x res*16/9, turned 90 degrees onto the window
   *               (console held vertically) -- sd_tate.c;
   *   rotation 3: portrait (res*9/16) x res, upright and 1:1 in the middle of
   *               the window, black sides -- sd_tate.c. */
  {
    const int p = sd_cfg_res, l = p * 16 / 9;
    if (sd_cfg_rotation == 1 || sd_cfg_rotation == 2) { screen_width = p;                screen_height = l; }
    else if (sd_cfg_rotation == 3)                    { screen_width = (p * 9 + 8) / 16; screen_height = p; }
  ...
  /* Asset pack FIRST: on the first boot this joins the .splitN chunks, packs
   * the ~3,800 loose files into assets.nxpack (with a progress screen), verifies
   * it and deletes the loose tree; on every later boot it just mounts the pack.
   * Done before check_data() so its lookups resolve through the pack. */
  const int packed = sd_pack_boot();
  check_data(packed);
   ...
   * uses POSIX signals Switch never delivers -> nativeRender would never return. */
  {
    ...
    /* Mode 1 is ENABLED, not "disabled" as this line used to log: SD2's
     * GarbageCollector::SetMode (il2cpp_gc_set_mode -> 0xf5c32c) calls GC_disable
     * for 0 and GC_enable for 1 (and 2, Manual), confirmed through the exported
     * il2cpp_gc_enable/_disable stubs (-> 0xfd243c / 0xfd24a8). The CALLS are kept
     * exactly as Sonic Dash 1 ran them on hardware; only the log is corrected.
     * Collections still happen later (SD1 logged them), and the GC bridge
     * (sd_gc.c) is what makes them safe. */
    if (il2cpp_gc_set_mode) { il2cpp_gc_set_mode(1); debugPrintf("[boot] il2cpp_gc_set_mode(1 = ENABLED)\n"); }
    else debugPrintf("[boot] WARNING: il2cpp_gc_set_mode not found\n");
    if (il2cpp_gc_disable)  { il2cpp_gc_disable();   debugPrintf("[boot] il2cpp_gc_disable() -> GC_disable (collections off until the engine re-enables them)\n"); }
  ...
  nx_input_set_il2cpp_api((void *)so_try_find_addr_rx(&il2cpp_mod, "il2cpp_array_new"),
                          (void *)so_try_find_addr_rx(&il2cpp_mod, "il2cpp_domain_get"),
                          (void *)so_try_find_addr_rx(&il2cpp_mod, "il2cpp_domain_get_assemblies"),
                          (void *)so_try_find_addr_rx(&il2cpp_mod, "il2cpp_assembly_get_image"),
                          (void *)so_try_find_addr_rx(&il2cpp_mod, "il2cpp_class_from_name"),
                          (void *)so_try_find_addr_rx(&il2cpp_mod, "il2cpp_class_array_element_size"));
  nx_install_input_hooks((uintptr_t)il2cpp_mod.load_virtbase);
  sd_uitrace_install(&il2cpp_mod);   /* log how taps resolve into clicks (config.txt ui_trace) */
  ...
  while (appletMainLoop() && !jni_quit_requested) {
    ...
    if (frame >= 60 && (frame % 120) == 60) nx_log_managed_time();   /* [time] every ~2 s */
    frame++;
