/* sd_patches.c -- engine patches and the engine clock, for Sonic Dash 2.
 *
...
/* How often the engine actually called TimeManager::Update (and so our clock):
 * bouncemasters_nx records, from cloverpit_nx, that on Unity 6 it may never
 * run. Logged by nx_log_managed_time() next to what C# reads from Time. */
static volatile unsigned g_tm_update_calls;
unsigned sd_tm_update_calls(void) { return g_tm_update_calls; }
