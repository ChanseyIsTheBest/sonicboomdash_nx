...
#include <GLES2/gl2.h>
#include <switch.h>
#include "sd_pack.h"   /* asset pack routing (access, pread, closedir) */
#include "diag.h"

#include "config.h"
#include "nx_pointer.h"   /* nxp_draw: the on-screen cursor (bloonspop_nx) */
#include "sd_tate.h"      /* rotated presentation (config.txt rotation) */
#include "so_util.h"
#include "util.h"
#include "libc_shim.h"
#include "sd_paths.h"
...
static int access_impl(const char *path, int mode) {
  ...
  (void)mode;
  if (sd_pack_exists(path)) return 0;          /* packed game file or directory */
...
static long pread_impl(int fd, void *buf, size_t n, long off) {
  long total = sd_pread(fd, buf, n, off);      /* libc_shim.c: pack-aware, loops short reads */
  watch_dump("pread", fd, (long)off, (long)n, buf, total < 0 ? 0 : total);
  return total;
}
...
/* sendfile(out, in, offset, count): Linux semantics, built on the port's own
 * read/pread/write so it works for real files AND asset-pack fds alike.
 *
 * FOUND ON HARDWARE: this was a stub returning -1 WITHOUT setting errno. .NET's
 * File.Copy (SystemNative_CopyFile) calls sendfile first and falls back to a
 * read/write loop only for EINVAL/ENOSYS -- so it read a stale errno (ENOENT)
 * and threw FileNotFoundException. Every save's backup copy failed:
 *   Unity: Unable to copy file System.IO.FileNotFoundException ...
 * With offset == NULL the input's file position advances, as on Linux;
 * otherwise *offset is used and updated and the position is left alone. */
long read_fake(int fd, void *buf, size_t count);
long write_fake(int fd, const void *buf, size_t count);
static long sendfile_stub(int out_fd, int in_fd, long *offset, size_t count) {
  static char buf[64 * 1024];            /* one caller at a time is the norm; guarded below */
  static Mutex lock;
  size_t done = 0;
  mutexLock(&lock);
  while (done < count) {
    size_t want = count - done < sizeof buf ? count - done : sizeof buf;
    long got = offset ? (long)pread_impl(in_fd, buf, want, *offset) : read_fake(in_fd, buf, want);
    if (got < 0) { if (!done) { mutexUnlock(&lock); return -1; } break; }
    if (got == 0) break;                                   /* end of input */
    long put = 0;
    while (put < got) {
      long w = write_fake(out_fd, buf + put, (size_t)(got - put));
      if (w <= 0) {                                        /* output refused: report what landed */
        if (offset) *offset += put;
        done += (size_t)put;
        mutexUnlock(&lock);
        if (!done) { if (w == 0) errno = EIO; return -1; }
        return (long)done;
      }
      put += w;
    }
    if (offset) *offset += got;
    done += (size_t)got;
  }
  mutexUnlock(&lock);
  return (long)done;
...
static EGLBoolean egl_QuerySurface_log(EGLDisplay d, EGLSurface s, EGLint attr, EGLint *val) {
  ...
  if (val) {
    /* Rotated, the engine must see its portrait size, never the landscape swapchain. */
    if (attr == 0x3057 /*EGL_WIDTH*/  && (*val <= 0 || sd_tate_enabled())) { *val = screen_width;  r = EGL_TRUE; }
    if (attr == 0x3056 /*EGL_HEIGHT*/ && (*val <= 0 || sd_tate_enabled())) { *val = screen_height; r = EGL_TRUE; }
  }
  return r;
}
...
static EGLBoolean egl_SwapBuffers_log(EGLDisplay d, EGLSurface s) {
  ...
  sd_tate_bind_overlay();   /* rotated: the cursor lands in the portrait FBO and turns with it */
  nxp_draw();               /* the on-screen cursor (bloonspop_nx); saves/restores all GL state */
  sd_tate_swap_hook();      /* rotated: one full-screen rotated pass onto the real window */
...
DynLibFunction dynlib_functions[] = {
  ...
  { "glBindBuffer", (uintptr_t)&glBindBuffer }, { "glBindFramebuffer", (uintptr_t)&sd_gl_BindFramebuffer_tate },
