/* sd_sdk_stubs.c -- in-process answers for the native SDK Sonic Dash 1 P/Invoked
 * (SEGA MariaUpload). SD2 does not use it -- its only P/Invoked SDK is Firebase
 * (firebase_stub.c) -- so these are inert here, kept only because they answer
 * by symbol name and cost nothing.
