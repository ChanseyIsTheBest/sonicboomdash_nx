/* nx_pointer.c -- from bloonspop_nx, unchanged apart from the rotation calls
 * (sd_tate_map_pointer / sd_tate_map_stick). */
...
#include <switch.h>
#include <GLES2/gl2.h>
#include <EGL/egl.h>          /* eglGetProcAddress: the GLES3 VAO entry points */
...
static void do_touch(void) {
  ...
  for (int i = 0; i < count; i++) {
    /* PANEL -> RENDER through the display rotation (Clay Jam): a plain axis
     * scale is wrong once the portrait image is rotated onto the panel. */
    float x, y;
    (void)sx; (void)sy;
    sd_tate_map_pointer((float)ts.touches[i].x, (float)ts.touches[i].y,
                        (float)s_cfg.panel_w, (float)s_cfg.panel_h, &x, &y);
    if (x < 0) x = 0;
    if (y < 0) y = 0;
...
/* For sd_tilt.c (the Enerbeam): the active controller's accelerometer, in G,
 * and which IMU frame it is in -- 1 Pro Controller, 2 handheld, 3 Joy-Con pair
 * -- chosen exactly as read_gyro() chooses the handle. */
int nxp_read_accel(float out[3], int *kind) {
  if (!s_gyro_ready) return 0;
  /* The handle the active controller names first, then the rest. A handle for
   * a controller that is not attached still returns a state -- all zeros --
   * and real accelerometer output always carries gravity (sonicjump_nx found
   * this on hardware), so a reading under 0.2 g is not a controller. */
  const u64 style = padGetStyleSet(&s_pad), attr = padGetAttributes(&s_pad);
  int order[8], n = 0;
  if (padIsHandheld(&s_pad) || (style & HidNpadStyleTag_NpadHandheld)) order[n++] = 0;
  if (style & HidNpadStyleTag_NpadFullKey) order[n++] = 1;
  if (style & HidNpadStyleTag_NpadJoyDual) {
    if (attr & HidNpadAttribute_IsRightConnected) order[n++] = 3;
    if (attr & HidNpadAttribute_IsLeftConnected)  order[n++] = 2;
  }
  for (int i = 0; i < 4; i++) order[n++] = i;          /* then anything live */
  for (int k = 0; k < n; k++) {
    const int i = order[k];
    HidSixAxisSensorState st; memset(&st, 0, sizeof st);
    if (hidGetSixAxisSensorStates(s_six[i], &st, 1) <= 0) continue;
    const float m2 = st.acceleration.x * st.acceleration.x + st.acceleration.y * st.acceleration.y +
                     st.acceleration.z * st.acceleration.z;
    if (m2 < 0.04f) continue;
    out[0] = st.acceleration.x; out[1] = st.acceleration.y; out[2] = st.acceleration.z;
    *kind = i == 0 ? 2 : i == 1 ? 1 : 3;                 /* handheld / Pro Controller / Joy-Con pair */
    return 1;
  }
  return 0;
}
/* The left stick, -1..1, x right and y UP (the pad nxp_poll() updated this frame). */
void nxp_stick_xy(float *x, float *y) {
  const HidAnalogStickState ls = padGetStickPos(&s_pad, 0);
  *x = (float)ls.x / 32767.0f; *y = (float)ls.y / 32767.0f;
}
/* Is this frame's input from the console's attached Joy-Cons -- the controller
 * that turns with the screen in the portrait modes? */
int nxp_is_handheld(void) { return padIsHandheld(&s_pad) ? 1 : 0; }
...
static void do_gyro(void) {
  ...
  if (fabsf(dx) > 0.05f || fabsf(dy) > 0.05f) {
    { float _rx, _ry; sd_tate_map_stick(dx, dy, &_rx, &_ry); s_cx += _rx;
    s_cy += _ry; }
    clamp_cursor();
  }
}
...
static int do_mouse(void) {
  ...
  if (dx || dy) {
    { float _rx, _ry; sd_tate_map_stick((float)dx * s_mouse_sens, (float)dy * s_mouse_sens, &_rx, &_ry); s_cx += _rx;
    s_cy += _ry; }
    clamp_cursor();
...
void nxp_update(void) {
  ...
  if (s_visible > 0) {
    ...
    if (ls.x || ls.y) {
      /* panel space first (stick +y is up, screen +y is down), then through
       * the display rotation so the cursor follows the picture. */
      float rdx, rdy;
      sd_tate_map_stick((ls.x / 32767.0f) * s_stick_speed,
                        -(ls.y / 32767.0f) * s_stick_speed, &rdx, &rdy);
      s_cx += rdx;
      s_cy += rdy;
      clamp_cursor();
    }
...
/* ---- the cursor's own vertex state ---------------------------------------------
 * The overlay draws inside the game's GL context at swap time. It used to draw
 * from CLIENT-MEMORY arrays inside whatever vertex array object Unity left bound,
 * saving and restoring only attributes 0 and 1. Any OTHER attribute Unity left
 * enabled in that VAO went into the draw too -- and when Unity deletes a buffer
 * such an attribute points into, GL reverts the attribute to client memory with
 * its old offset as the "pointer". Nouveau then dereferenced that offset
 * (crash report 01789148881: nvc0_update_user_vbufs_shared <- glDrawArrays <-
 * nxp_draw <- eglSwapBuffers). Now the cursor has its own VAO -- only our two
 * attributes exist in it -- and its own VBO, so the client-memory path is never
 * taken; Unity's VAO is rebound afterwards, untouched. Without VAO support the
 * fallback disables every other enabled attribute for the draw and restores it. */
extern int debugPrintf(char *text, ...);   /* util.c */
typedef void (*PFN_GenVA)(GLsizei, GLuint *);
typedef void (*PFN_BindVA)(GLuint);
static PFN_BindVA s_bind_va;
static GLuint s_vao, s_vbo;
static int s_vstate_tried;
static void vstate_init(void) {
  if (s_vstate_tried) return;
  s_vstate_tried = 1;
  glGenBuffers(1, &s_vbo);
  PFN_GenVA gen = (PFN_GenVA)eglGetProcAddress("glGenVertexArrays");
  s_bind_va = (PFN_BindVA)eglGetProcAddress("glBindVertexArray");
  if (gen && s_bind_va) gen(1, &s_vao);
  if (!s_vao) s_bind_va = NULL;
  debugPrintf("[nxp] cursor vertex state: own VBO %u, own VAO %u%s\n", s_vbo, s_vao,
              s_vao ? "" : " (no VAO support: other attributes are disabled per draw instead)");
}
...
void nxp_draw(void) {
  ...
  vstate_init();
  if (!s_vbo) return;                                   /* no buffer: never fall back to client memory */

  /* ---- save every bit of state we touch ---- */
  AttribState a0, a1;
  GLint prev_vao = 0;
  unsigned char other_on[32] = { 0 };
  if (s_vao) {
    glGetIntegerv(GL_VERTEX_ARRAY_BINDING, &prev_vao);
    s_bind_va(s_vao);                                   /* only our attributes exist in here */
  } else {
    attrib_save(0, &a0);
    attrib_save(1, &a1);
    GLint max_attr = 0;
    glGetIntegerv(GL_MAX_VERTEX_ATTRIBS, &max_attr);
    for (GLint i = 2; i < max_attr && i < 32; i++) {
      GLint on = 0;
      glGetVertexAttribiv((GLuint)i, GL_VERTEX_ATTRIB_ARRAY_ENABLED, &on);
      if (on) { other_on[i] = 1; glDisableVertexAttribArray((GLuint)i); }
    }
  }
  ...
  /* Draw across the WHOLE render target. The cursor maps pixels through
   * screen_w x screen_h, but the viewport at swap time is whatever the game
   * left: Sonic Dash 1 at its 75% scale left 960x540 on a 1280x720 window,
   * which squeezed the whole cursor into the bottom-left 75% of the screen.
   * (bloonspop_nx never met this: its cursor always draws into a framebuffer
   * whose viewport already matches.) Restored below. */
  GLint prev_vp[4];
  ...
  if (s_cursor_tex) {
    ...
    const GLfloat verts[] = { 0,0,  w,0,  0,h,  w,h,       /* positions */
                              0,0,  1,0,  0,1,  1,1 };     /* uv        */
    ...
    glBindBuffer(GL_ARRAY_BUFFER, s_vbo);              /* our buffer, never client memory */
    ...
    glUniform1f(s_u_scale, (GLfloat)(s_cfg.screen_w < s_cfg.screen_h ? s_cfg.screen_w : s_cfg.screen_h) / 1080.0f);
    ...
    glBufferData(GL_ARRAY_BUFFER, sizeof s_arrow, s_arrow, GL_STREAM_DRAW);
    glEnableVertexAttribArray(0);
    glDisableVertexAttribArray(1);                     /* a stale uv attribute must not be read */
    glVertexAttribPointer(0, 2, GL_FLOAT, GL_FALSE, 0, (const void *)0);
    glUniform1f(s_u_use_tex, 0.0f);                    /* shader will not sample */

    /* shorter side: a portrait render must not size the arrow by its 1280 "height" */
    const GLfloat sc = 2.4f * ((GLfloat)(s_cfg.screen_w < s_cfg.screen_h ? s_cfg.screen_w : s_cfg.screen_h) / 1080.0f);
  ...
  if (s_vao) {
    s_bind_va((GLuint)prev_vao);                        /* Unity's VAO, exactly as it was */
  } else {
    attrib_restore(0, &a0);
    attrib_restore(1, &a1);
    for (int i = 2; i < 32; i++) if (other_on[i]) glEnableVertexAttribArray((GLuint)i);
  }
